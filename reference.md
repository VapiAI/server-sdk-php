...
**$purpose:** `?string` — Only return files with this purpose. When omitted, files of every purpose except composer attachments are returned.
...
**$assistant:** `?CreateAssistantDto` — Complete assistant replacement. Omitted credentials and redacted server secrets are preserved when their endpoint URL is unchanged. Send credentials: [] to clear credentials; omit a server container to remove it.
...
**$targetOverrides:** `?AssistantOverrides` — Complete override replacement. Omitted credentials and redacted server secrets are preserved when their endpoint URL is unchanged. Send credentials: [] to clear credentials; omit a server container to remove it.
...
**$search:** `?string` — Literal, case-insensitive search across eval and assistant names.
