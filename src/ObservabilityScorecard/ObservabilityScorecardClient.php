...
namespace Vapi\ObservabilityScorecard;

use Psr\Http\Client\ClientInterface;
use Vapi\Core\Client\RawClient;
use Vapi\Types\Scorecard;
...
use Vapi\ObservabilityScorecard\Requests\UpdateScorecardDto;
use Vapi\ObservabilityScorecard\Requests\ScorecardControllerGetPaginatedRequest;
use Vapi\Types\ScorecardPaginatedResponse;
use Vapi\Core\Json\JsonSerializer;
use Vapi\Types\CreateScorecardDto;
...
class ObservabilityScorecardClient
{
     ...
     */
    public function scorecardControllerGet(string $id, ?array $options = null): ?Scorecard
    {
        ...
        try {
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return Scorecard::fromJson($json);
            }
     ...
     * @throws VapiApiException
     */
    public function scorecardControllerRemove(string $id, ?array $options = null): ?Scorecard
    {
        $options = array_merge($this->options, $options ?? []);
        try {
            $response = $this->client->sendRequest(
     ...
     * @throws VapiApiException
     */
    public function scorecardControllerUpdate(string $id, UpdateScorecardDto $request = new UpdateScorecardDto(), ?array $options = null): ?Scorecard
    {
        $options = array_merge($this->options, $options ?? []);
        try {
            $response = $this->client->sendRequest(
     ...
     *   bodyProperties?: array<string, mixed>,
     * } $options
     * @return ?ScorecardPaginatedResponse
     * @throws VapiException
     * @throws VapiApiException
     */
    public function scorecardControllerGetPaginated(ScorecardControllerGetPaginatedRequest $request = new ScorecardControllerGetPaginatedRequest(), ?array $options = null): ?ScorecardPaginatedResponse
    {
        $options = array_merge($this->options, $options ?? []);
        $query = [];
        if ($request->id != null) {
        ...
        try {
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return ScorecardPaginatedResponse::fromJson($json);
            }
        } catch (JsonException $e) {
            throw new VapiException(message: "Failed to deserialize response: {$e->getMessage()}", previous: $e);
        } catch (ClientExceptionInterface $e) {
     ...
     * @throws VapiApiException
     */
    public function scorecardControllerCreate(CreateScorecardDto $request, ?array $options = null): ?Scorecard
    {
        $options = array_merge($this->options, $options ?? []);
        try {
            $response = $this->client->sendRequest(
